...
本章のハンズオンは概念理解を優先する最小例です。本番やチーム検証へ持ち込む前に、次の観点を追加で確認してください。
Pod Security Admission は namespace 単位で Pod Security Standards を適用する仕組みであり、`privileged` / `baseline` / `restricted` の各レベルを意識して設計します。

| 観点 | 最低限の確認 |
| --- | --- |
| securityContext | イメージが対応する範囲で `runAsNonRoot`、`allowPrivilegeEscalation: false`、`seccompProfile.type: RuntimeDefault`、capabilities の削減を検討する |
| ServiceAccount / RBAC | Pod に不要な API 権限を持たせない。`default` ServiceAccount をそのまま使う場合は権限が付与されていないか確認する |
| Namespace | 学習用 namespace と本番/共有 namespace を分け、Pod Security Admission の label 適用対象を確認する |
| resources | requests/limits を設定し、ノード逼迫時の挙動と QoS クラスを理解する |

注意: 既存イメージに `runAsNonRoot` 等を後付けすると起動できない場合があります。
セキュリティ設定は「付ければ安全」ではなく、イメージのユーザー設計やファイル権限とセットで確認します。
