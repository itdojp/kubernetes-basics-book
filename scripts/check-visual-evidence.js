...
const EXPECTED = [
  ['ch00-podman-version', 'chapter00', 'ch00-podman-version-01.png'],
  ['ch00-nginx-http', 'chapter00', 'ch00-nginx-http-02.png'],
  ['ch01-api-explore', 'chapter01', 'ch01-kubectl-explore-01.png'],
  ['ch02-kind-bootstrap', 'chapter02', 'ch02-kind-bootstrap-01.png'],
  ['ch03-label-selector', 'chapter03', 'ch03-kubectl-show-labels-01.png'],
  ['ch04-pod-design', 'chapter04', 'ch04-kubectl-describe-pod-01.png'],
  ['ch05-rollout', 'chapter05', 'ch05-rollout-rs-01.png'],
  ['ch06-service-dns', 'chapter06', 'ch06-service-dns-01.png'],
  ['ch07-historical-ingress', 'chapter07', 'ch07-ingress-nginx-01.png'],
  ['ch08-config-secret', 'chapter08', 'ch08-configmap-secret-01.png'],
  ['ch09-pvc', 'chapter09', 'ch09-pvc-mount-01.png'],
  ['ch10-selector-debug', 'chapter10', 'ch10-service-selector-debug-01.png'],
];
...
const FORBIDDEN = [
  ...
  { label: 'capture cluster identity', pattern: /(?:kind-book-qa|book-qa-control-plane)/i },
  { label: 'ephemeral Secret value', pattern: /(?:issue-13-ephemeral-secret|change-me)/i },
...
function validateVisualEvidence(repoRoot = path.resolve(__dirname, '..')) {
  ...
  if (manifest.issue !== 13) errors.push('manifest issue must be 13');
  ...
  if (!/^https:\/\/github\.com\/itdojp\/kubernetes-basics-book\/actions\/runs\/\d+$/.test(manifest.captureRun || '')) {
  ...
  entries.forEach((entry, index) => {
    ...
    ids.add(entry.id);
    if (!expected || entry.id !== expected[0] || entry.chapter !== expected[1]) errors.push(`${label}: entry order/id/chapter differs from the fixed 12-item P0 inventory`);
